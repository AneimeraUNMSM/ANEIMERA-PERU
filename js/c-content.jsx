/* Full content system for Prototipo-C.html — includes utilities + Edit JSX component */

/* Redefine or import from c-content-core.js equivalent — we duplicate for independence */
...
function deepMerge(base, over) {
  ...
  const out = Array.isArray(base) ? [...base] : { ...base };
  for (const k of Object.keys(over)) {
    out[k] = (typeof base?.[k] === "object" && base[k] !== null) ? deepMerge(base[k], over[k]) : over[k];
  }
  return out;
}
...
/* Edit component — renders content with CMS path attribute for inline editing */
function Edit({ path, as: Tag = "span", children, ...rest }) {
  return <Tag data-cms-path={path} {...rest}>{children}</Tag>;
}
window.Edit = Edit;
